...
def commLinkInfoTable(link, StartTime, StopTime, Step, satellite):
    ...
    duration_1 = access_data_results.DataSets.GetDataSetByName('Duration').GetValues()
    ...
    return tabla, accessStartTime, accessStopTime, duration_1
    
def commLinkInfoTableSyncTimes(link, start_time, stop_time, Step, satellite):  
...
acces_name = scenario_metadata["receivers"]["saocom1a_receiver"]["receiver_parent"] + "_acces_" + scenario_metadata["receivers"]["saocom1a_receiver"]["link_transmitters"][0]
...
ts_name = scenario_metadata["receivers"]["saocom1a_receiver"]["link_transmitters"][0]
dem = scenario_metadata["receivers"]["saocom1a_receiver"]["dem"]
rs_name = scenario_metadata["receivers"]["saocom1a_receiver"]["name"]
rs_sat_name = scenario_metadata["receivers"]["saocom1a_receiver"]["receiver_parent"]
...
tabla = commLinkInfoTableSyncTimes(link=Access[acces_name], start_time=accessStartTime[longest_duration_idx], stop_time=accessStopTime[longest_duration_idx], Step=StepTime, satellite=Satellites[rs_sat_name].sat)
...
tabla = commLinkInfoTableSyncTimes(link=Access[acces_name], start_time=new_start_time, stop_time=new_stop_time, Step=StepTime, satellite=Satellites[rs_sat_name].sat)
reporte = pd.DataFrame(tabla)
reporte_len = len(reporte['Access Number'])
reporte['Modulation'] = [dem] * reporte_len
reporte.to_excel("Reports/" + report_name+".xlsx")
